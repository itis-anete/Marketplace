  ...
  <v-container fluid class="pl-0 pb-0 pt-0 pr-0" v-if="market">
    <v-container class="panel" fluid>
      <v-layout row wrap justify-center align-center>
        <v-flex xs11 v-if="market">
          <v-layout justify-center>
            <v-flex xs1></v-flex>
            <v-flex xs4>
              <v-card>
                <v-card-text class="accent white--text text-xs-center display-2">{{market.name}}</v-card-text>
              </v-card>
            </v-flex>
          </v-layout>
        </v-flex>
      ...
      <v-navigation-drawer v-model="drawer" absolute temporary right>
        ...
        <v-list class="pt-0" dense>
          ...
          <template>
            <edit-market :market="market"></edit-market>
          </template>
        </v-list>
      </v-navigation-drawer>
    </v-container>
    <v-container grid-list-xs fluid>
      ...
      >
        <v-flex
          xs12
          sm2
          v-for="productOffer in market.productOffers"
          :key="productOffer.id"
          class="mr-4 mb-4"
        ...
        >
          <router-link :to="'/productOffer/' + productOffer.id">
            <v-card class="pointer">
              ...
              <v-card-text class="headline pb-0">{{productOffer.product.name}}</v-card-text>
              <v-card-text class="display-1 pt-0 mt-0">
                <span class="accent--text">{{productOffer.price | price}}</span>
                <span class="headline">руб.</span>
              </v-card-text>
            </v-card>
          </router-link>
        </v-flex>
...
.panel {
  box-shadow: inset 0px -2px 0px 0px #039be5,
