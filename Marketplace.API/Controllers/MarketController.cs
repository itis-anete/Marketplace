...
namespace Marketplace.API.Controllers
{
    ...
    public class MarketController : Controller
    {
        ...
        {
            return await _unitOfWork.MarketRepository.Get().Include(c => c.ProductOffers).ThenInclude(c=>c.Product).ToListAsync();
        }

        [HttpGet("[action]/{marketId}")]
        public async Task<Market> GetMarketById(int marketId)
        {
            return await _unitOfWork.MarketRepository.Get(x=>x.Id == marketId).Include(c => c.ProductOffers).ThenInclude(c => c.Product).FirstOrDefaultAsync();
        }

        [HttpPost("[action]")]
        public async Task<IActionResult> AddMarket([FromBody]Market market)
        {
            if (_unitOfWork.MarketRepository.Get().Any(x => x.Name == market.Name)) return BadRequest(market);
